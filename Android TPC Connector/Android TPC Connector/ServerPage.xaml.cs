...
namespace Android_TPC_Connector
{
    ...
    public partial class ServerPage : ContentPage
    {
        ...
        private void disconnectBtn_Clicked(object sender, EventArgs e)
        {
            ...
            foreach (var d in ClientDisplayers)
                if (d.Client == client)
                { client.SendString("MANUAL DISCONNECT"); ClientDisplayers.Remove(d); clients.Remove(id); Device.BeginInvokeOnMainThread(() => { clientLister.ItemsSource = null; clientLister.ItemsSource = ClientDisplayers; }); break; }
        }
    }
}