    ...
    namespace CSharp
    {
        ...
        namespace DataTypeExtensions
        {
            ...
            public static class GeneralClassExtensions
            {
                ...
                {
                    if (Regex.IsMatch(v, @"\d+")) try { return int.Parse(v); } catch { return 0; }
                    else if (Regex.IsMatch(v, @"[\dA-Fa-f]+")) return Converter.HexadecimalToDecimal(v);
                    else return 0;
                }
                public static long ToLong(this string v)
                {
                    if (Regex.IsMatch(v, @"\d+")) try { return long.Parse(v); } catch { return 0; }
                    else if (Regex.IsMatch(v, @"[\dA-Fa-f]+")) return (long)Converter.HexadecimalToDecimal(v);
                    else return 0;
                }
        ...
        namespace Data
        {
            ...
            public static class Resource
            {
                ...
                public static string GetEmbeddedTextFileContent(string ResourcePath)
                {
                    ...
                    {
                        r.BaseStream.Position = pos;
                        string posText = CSharp.Converter.HexadecimalToText(r.ReadByte().ToString("X2"));
                        FileText += posText;
                        pos++;
                    }
                    r.Close();
...
#pragma warning restore IDE0060 // Remover o parâmetro não utilizado
            ...
            public static string RootOfAsString(int NumberToGetRoot, int Index = 2, int DecimalPlaces = 0)
            {
                ...
                while (dPlaces < DecimalPlaces)
                {
                    for (double i = 0; i < NumberToGetRoot; i += double.Parse(saddN))
                    {
                        var next = i + double.Parse(saddN);
                        var sum = next + double.Parse(Root.Replace(",", "."));
                        var power = Math.Pow(sum, Index);
                        if (power > NumberToGetRoot)
                        {
                            Root += (sum - double.Parse(saddN)).ToString().Last().ToString();
            ...
            public static string RootOfAsString(long NumberToGetRoot, int Index = 2, int DecimalPlaces = 0)
            {
                if (NumberToGetRoot < 0 && Index % 2 == 0)
                {
                    return "0";
...
#pragma warning restore IDE0017 // Simplificar a inicialização de objeto
            ...
            public static class FileInfo
            {
                ...
                {
                    if (new CSharp.IO.HexDumpStream(firstFile).ReadAll() == new CSharp.IO.HexDumpStream(secondFile).ReadAll()) { return true; } else { return false; }
                }
